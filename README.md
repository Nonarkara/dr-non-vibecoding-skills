...
A complete, production-hardened stack for pairing with AI agents — plain markdown, zero runtime dependencies. A battle-tested collection of 168 skills, 16 playbooks, 13 references, and 17 drop-in templates that bridge the gap between human intuition and shipped software. Proven on live civic sensor grids, and architected for any resilient business.
...
[![Skills: 168](https://img.shields.io/badge/skills-168-F59E0B)](skills/)
...
**168 skills** · **16 playbooks** · **13 references** · **17 templates**
...
- **[`playbooks/16-the-philosophical-spine.md`](playbooks/16-the-philosophical-spine.md)** — the philosophical *why* beneath the collection. The 5 layers, the 5 moves, the 8 cyber-hygiene rules, the 6 off-grid rules.
...
├── skills/           # 168 modular agent capabilities (plain markdown with YAML frontmatter)
...
├── CATALOG.md        # The complete categorized routing inventory of all 168 skills
...
|---|---|---|---|
| [`skills/`](skills/) | 168 `SKILL.md` files | Standing operational instructions loaded on demand | Agent reads the file when triggered by a specific task or keyword |
...
| REST API contracts that survive clients in the wild | [`api-design`](skills/api-design/SKILL.md) | Resource modeling as nouns, cursor pagination, idempotency keys, single error envelope, and the REST-vs-GraphQL decision rule. |
| Caching across edge, memory, Redis, and DB | [`caching-layers`](skills/caching-layers/SKILL.md) | Cache-aside vs write-through, stale-while-revalidate, thundering-herd protection, and the CDN-lies deploy rule. |
| Webhook receivers that don't lie to you | [`webhooks-reliable`](skills/webhooks-reliable/SKILL.md) | Always-200-fast then async-process; idempotency by `(sender, event_id)`; signature verification with `timingSafeEqual`; exponential backoff + jitter; DLQ that pages a human. |
...
If it changed layout, spacing, type, color, or motion, screenshot it and critique it with $see-and-revise before shipping.
