...
|---|---|---|---|
| [01](01-stack.md) | **STACK** — the package at a glance | 75 skills, 14 playbooks, 7 references, 13 templates is **one product**, not four. Plain markdown, no runtime, agent-agnostic. The routing loop is a habit, not a diagram. | You treat the repo as a skills library instead of a stack. You add skills without knowing the loop they belong to. |
| [03](03-route.md) | **ROUTE** — the smallest useful path | "The catalog is a router." Eight moments, eight routes. The 75 skills collapse to 8 if you are in a hurry. **Route, don't scan.** | You load all 75 skills into context on every change. You spend the first ten minutes of every session finding where to start. |
