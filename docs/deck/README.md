...
|---|---|---|---|
| [01](01-stack.md) | **STACK** — the package at a glance | 69 skills, 13 playbooks, 7 references, 12 templates is **one product**, not four. Plain markdown, no runtime, agent-agnostic. The routing loop is a habit, not a diagram. | You treat the repo as a skills library instead of a stack. You add skills without knowing the loop they belong to. |
| [03](03-route.md) | **ROUTE** — the smallest useful path | "The catalog is a router." Eight moments, eight routes. The 69 skills collapse to 8 if you are in a hurry. **Route, don't scan.** | You load all 69 skills into context on every change. You spend the first ten minutes of every session finding where to start. |
