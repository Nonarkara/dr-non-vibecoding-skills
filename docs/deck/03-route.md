...
> **The catalog is a router.** Start with the moment you are in. Load the smallest useful skill set. Do not scan all 69.

A router collapses 69 skills to 8. If you know the moment, you know the route. The page is a cheat sheet for the 8 moments a working day actually contains.
...
## Why "load one for the decision" instead of "load all 69"

Three reasons, in order of cost.

**Context cost.** Loading 69 skill descriptions into the agent's context window on every change is not free. Most of the skill body is not relevant to the current decision. The router pattern keeps the loaded context small (typically 1-2 skills) by loading the *one* skill that matches the *current* moment.
...
**Discovery cost.** A new contributor cannot memorise 69 skills. They can memorise 8 moments. The route is the entry point; the skill is the deep dive.
