...
A router collapses 167 skills to 8. If you know the moment, you know the route. The page is a cheat sheet for the 8 moments a working day actually contains.
...
## Why "load one for the decision" instead of "load all 167"

Three reasons, in order of cost.

**Context cost.** Loading 167 skill descriptions into the agent's context window on every change is not free. Most of the skill body is not relevant to the current decision. The router pattern keeps the loaded context small (typically 1-2 skills) by loading the *one* skill that matches the *current* moment.
...
**Discovery cost.** A new contributor cannot memorise 167 skills. They can memorise 8 moments. The route is the entry point; the skill is the deep dive.
