...
The model is the loudest variable in the system. Every six months a new model ships, and the practice's apparent relevance is questioned. **The model is not the system.** The system is the four shapes wired into one routing loop, with the 69 skills carrying the load-bearing judgments and the 12 templates carrying the load-bearing files.
...
The 69 skills are the lookups. The 13 playbooks are the receipts for the lookups. The 7 references are the stable implementation details. The 12 templates are the drop-in files. The whole collection is the **decision ledger** that compounds.
